class TransactionsController < ApplicationController
  ...
  def create
    ...
    total_investment = current_users_stocks.pluck(:investment).inject(:+)
    total_quantity = current_users_stocks.pluck(:quantity).inject(:+)
    avg_lbp = current_users_stocks.pluck(:last_buying_price).inject(:+)
    avg_abp = current_users_stocks.pluck(:average_buying_price).inject(:+)
    total_investment_percentage = (current_users_stock.investment / total_investment) * 100

    render json: {
      user_stock_id: current_users_stock.id,
      name: current_users_stock.stock.name,
      investment: current_users_stock.investment.floor,
      investment_percentage: total_investment_percentage.truncate(1).to_f,
